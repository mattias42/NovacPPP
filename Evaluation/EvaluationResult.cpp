...
static int nObjectsAllocated = 0; // this is for debugging only!
...
CEvaluationResult::CEvaluationResult(const CEvaluationResult &b)
{
	...
	for(long i = 0; i < nRef; ++i){
		...
		ref.m_specieName.Format("%s", (const char*)b.m_ref[i].m_specieName);
...
CEvaluationResult::~CEvaluationResult(void)
{
	m_ref.RemoveAll();
	m_ref.FreeExtra();
	m_correction.RemoveAll();
	m_correction.FreeExtra();
