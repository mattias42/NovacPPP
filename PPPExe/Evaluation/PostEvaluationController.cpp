...
//	reader.ReadSpectrum(pakFileName, 0, spec); // TODO: check for errors!!
	...
	if(Equals(fitWindow.name, g_userSettings.m_fitWindowsToUse[g_userSettings.m_mainFitWindow])){
		if(0 == CheckQualityOfFluxMeasurement(m_lastResult, pakFileName)){
			delete scan;
			delete ev;
			delete m_lastResult;	m_lastResult = NULL;
			return 6;
		}
	}
...
//	string.AppendFormat("\tplumeheight=%.2lf\n",		windField.GetPlumeHeight());
	...
	if(sky.m_length > 0){
		...
		CEvaluationLogFileHandler::FormatEvaluationResult(&sky.m_info,	NULL, instrLocation->m_instrumentType, maxIntensity*sky.NumSpectra(), window->nRef, string1);
	}
	scan->GetDark(dark);
	...
	if(dark.m_length > 0){
		...
		CEvaluationLogFileHandler::FormatEvaluationResult(&dark.m_info, NULL, instrLocation->m_instrumentType, maxIntensity*dark.NumSpectra(), window->nRef, string2);
	}
	...
	if(offset.m_length > 0){
		...
		CEvaluationLogFileHandler::FormatEvaluationResult(&offset.m_info, NULL, instrLocation->m_instrumentType, maxIntensity * offset.NumSpectra(), window->nRef, string3);
	}
	...
	if(darkCurrent.m_length > 0){
		...
		CEvaluationLogFileHandler::FormatEvaluationResult(&darkCurrent.m_info, NULL, instrLocation->m_instrumentType, maxIntensity*darkCurrent.NumSpectra(), window->nRef, string4);
	}
