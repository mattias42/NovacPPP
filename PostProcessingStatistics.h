...
public:
	...
	/** Inserts information on a rejected scan from a certain instrument 
		into the database. */
	void InsertRejection(const CString &serial, const REASON_FOR_REJECTION &reason);

	/** Inserts information on a accepted scan from a certain instrument into the database. */
	void InsertAcception(const CString &serial);

	/** Retrieves the number of rejected full scans due to the specified reason */
	unsigned long GetRejectionNum(const CString &serial, const REASON_FOR_REJECTION &reason);
	
	/** Retrieves the number of accepted full scans */
	unsigned long GetAcceptionNum(const CString &serial);
	...
	void WriteStatToFile(const CString &file);
